(function() {
  ...
  function createFireSource() {
    for(let col = 0; col <= fireWidth; ++col) { //understand why <= instead of <
  ...
  function calculateFirePropagation()
  {
    for(let col = 0; col < fireWidth; ++col)
    {
      for(let row = 0; row < fireHeight; ++row)
      {
        ...
        updateFireIntensityByPixel(pixelIndex);
      }
    }
  ...
  function updateFireIntensityByPixel(currentPixelIndex)
  {
    ...
    firePixelsArray[currentPixelIndex - decay] = newFireIntensity < 0 ? 0 : newFireIntensity;
  }
    ...
    for(let row = 0; row < fireHeight ; ++row)
    {
      ...
      for(let col = 0; col < fireWidth ; ++col)
      {
        ...
        if (debug) {          
          ...
          tableCol.classList.add("fire-table__column");
          ...
          tableCol.append(colIndex);
          tableCol.append(fireIntensity);
        } else {
          const color = fireColorsPalette[fireIntensity];
          const tableCol = document.createElement("td");
          tableRow.append(tableCol);

          tableCol.classList.add("fire-table__column-fire");
          tableCol.style.backgroundColor = `rgb(${color.r},${color.g},${color.b})`;
        }
      }
    }
